...
public class VatGenerator extends SpecialBlockStateGen {
    ...
    public VatGenerator(String prefix) {
        this.prefix = prefix;
    }
    ...
    public <T extends Block> ModelFile getModel(DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov,
                                                BlockState state) {
        Boolean top = state.getValue(VatBlock.TOP);
        Boolean bottom = state.getValue(VatBlock.BOTTOM);
        VatBlock.Shape shape = state.getValue(VatBlock.SHAPE);
        ...
        String modelName = shapeName + (shape == VatBlock.Shape.PLAIN ? "" : "_" + shape.getSerializedName());
